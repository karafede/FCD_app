...
# setwd("D:\\Federico\\FCDapp_mobile")
...
shinyServer(function(input, output, session) {
    ...
    gc()
    DB <- input$database  
    DB1 <- input$database_1  ## for map-matching
    ...
    #                        user = "postgres", password = "superuser")
                   ...
                   TABLE1 <- reactive({
                    ...
                    gc()
                     ...
                     ###---rename data
                     count_vehtype$vehtype <- gsub(1, "cars", (count_vehtype$vehtype))
                     count_vehtype$vehtype <- gsub(2, "fleet", (count_vehtype$vehtype))
                ...
                ## bias position (end - start new trip)
                 output$plot_offset_position <- renderPlot({
                   ...
                   output$plot_hourly_distance_profile <- renderPlot({
                     ...
                     #   # req(input$route_BRESCIA)
                     #   # route <- readRDS(input$route_BRESCIA$datapath)
                     # } else if (DB1 == 'HAIG_ROMA') {
                     #   route <- route_ROMA
                     #   # req(input$route_ROMA)
                     #   # route <- readRDS(input$route_ROMA$datapath)
                     # } else if (DB1 == 'HAIG_CATANIA') {
                     #   route <- route_CATANIA
                     # } else if (DB1 == 'HAIG_SALERNO') {
                     #   route <- route_SALERNO
                     # } else {print("no matched data available")}
                   ...
                   output$plot_daily_distance_profile <- renderPlot({
                     ...
                     route$speed_trip = round((route$tripdistance_m/1000)/(route$triptime_s/3600), digits = 0)  ## km/h
                     route$hour <- hour(route$timedate_o)
                     route$day <- (c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", 
                       "Friday", "Saturday")[as.POSIXlt(route$timedate_o)$wday + 1])
                     route$month <- month(as.Date(route$timedate_o, tz = "CET"))
                     ...
                     ### summary statistics by HOUR
                     ### find the number trips for each daily traveled distance
                     
                     trips_weekdays <- route %>%
                       group_by(month, day) %>%
                       summarise(count = length(idtrajectory),
                                 total_km = sum(tripdistance_m/1000))
                     ...
                     month_to_remove <- AAA$month[AAA$count < 7]
                     ...
                     trips_weekdays$day <- ordered(trips_weekdays$day, levels=c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday" ,"Saturday" ,"Sunday"))
                     trips_weekdays <- trips_weekdays[order(trips_weekdays$day), ]
                     remove(trips_month)
                     
                     
                     Figure_daily_distances <- ggplot(data = trips_weekdays,
                       ...
                       # geom_text(aes(label = paste(count, sep = "")), size = 5, hjust = 0.5, vjust = -0.5) +
                       ggtitle("Daily trip distance") + 
                       theme(plot.title = element_text(lineheight=.8, face="bold", size = 14))
                     
                     return(Figure_daily_distances)
                   })
                 ...
                 finalMap <- reactive({
                   ...
                   mapmatching <- mapmatching %>%
                     ...
                     filter(`load(%)` >= 17)
                   mapmatching$counts <- (mapmatching$counts / max(mapmatching$counts))*2
                   ...
                   sf <- sf::st_as_sf(mapmatching, wkt = "geometry" )
                   # plot(sf)
                   ...
                   if (DB == 'HAIG_BRESCIA') {
                     ...
                     map <- leaflet() %>%
                       setView(10.310790077709969, 45.57001466313041, 12) %>%  
                       addProviderTiles(providers$CartoDB.Positron) %>%
                       # addTiles(group = "OSM (default)") %>%
                       addPolylines(data = edges, 
                                    color='blue', 
                                    group='edge',
                                    weight = mapmatching$counts,
                                    label = paste0(mapmatching$name, "  ", 
                                                   ", load(%): ", mapmatching$`load(%)`))
                     map
                     
                     
                     } else if (DB == 'HAIG_ROMA') {
                       map <- leaflet() %>%
                         setView(12.723693130461931, 41.78717550303782, 10) %>%  
                         addProviderTiles(providers$CartoDB.Positron) %>%
                         addPolylines(data = edges, 
                                      color='blue', 
                                      group='edge',
                       ...
                       map <- leaflet() %>%
                         setView( 15.12, 37.51417356429861, 12) %>%  
                         addProviderTiles(providers$CartoDB.Positron) %>%
                         addPolylines(data = edges, 
                                      color='blue', 
                                      group='edge',
                       ...
                       map
                         ...
                         map <- leaflet() %>%
                           setView(14.85, 40.71702143144529, 12) %>%
                           addProviderTiles(providers$CartoDB.Positron) %>%
                           addPolylines(data = edges, 
                                        color='blue', 
                                        group='edge',
                 ...
                 sep="\n"
                   ...
                   output$text_table2 <- renderText({ 
                     paste("Structure of 'routes' obtained from the elaboration of Floating Car Data (FCD)")
                   })
                   
                   output$table_2 <- DT::renderDataTable(TABLE2())
                   
                   
                   output$text_maps <- renderText({ 
                     paste("Roads with HIGH traffic loading during a typical working day.")
                   })
                   
                   
                   
                   output$author <- renderText({ 
                     paste("create by Federico Karagulian @ENEA.")
                   })
